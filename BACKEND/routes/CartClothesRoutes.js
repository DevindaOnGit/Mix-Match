...
router.route("/item/:index").get(async (req, res) => {
    try {
        ...
        // Fetch the clothes sorted by _id, skipping the appropriate number of entries
        const clothesList = await clothes.find().sort({ _id: 1 }).skip(index).limit(1);

        if (clothesList.length > 0) {
            res.json(clothesList[0]); // Return the first (and only) item from the list
        } else {
            res.status(404).json({ error: "No more clothes found" });
        }
    } catch (error) {
